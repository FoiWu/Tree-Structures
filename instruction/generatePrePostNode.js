//
//Press a button to display a node of pre-order or post-order.	
...
var generatePrePostNode = function(){
	...
	　		var k=Math.floor(Math.random()*7);
		...
		if( kind == 1 ){
			for( var i = 0; i <= 6; i++ ){
				dataArray[i] = preorderArr[i];
			}
		}
		else if( kind == 2 ){
			for( var i = 0; i <= 6; i++ ){
				dataArray[i] = postorderArr[i];
			}
		}
	}
...
var genPrePost = new generatePrePostNode();