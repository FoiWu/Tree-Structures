...
var outlineIndex = 2, appearArrIndex = 0, oldOutline, leftChild, corPresent;
...
function drag(element, event){
	if (dragTarget){
		...
		for( var k = 0; k <= placeTextId.length; k++ ){
			//alert(placeTextId+'/'+dragTarget.id);
			if( placeTextId[k] == dragTarget.id ){
				colorAnswer[k] = null;   
				break;    	
			}	
		}
...
function drop(element, event){
	if (dragTarget){
		...
		if(targetElement != treeSVG.node()){
			...
			for( corPresent = 1; corPresent <= outlineIndex; corPresent++ ){
				if( targetElement.id == "outline"+corPresent ){
					leftChild = corPresent*2;
					break;
				}	
			}
			...
			$(document).ready(function() { 
				$(function() {
					$( "#dialog-confirm" ).dialog({
						...
						buttons: {
						   "Red": function() {
							  d3.select(targetElement).attr("fill", "none").attr("stroke-width", 6).attr("stroke", "red");
							  colorAnswer[(corPresent-1)] = "Red";
							  $( this ).dialog( "close" );
							  console.log(colorAnswer);
							},
						   "Black": function() {
							  d3.select(targetElement).attr("fill", "none").attr("stroke-width", 6).attr("stroke", "black");
							  colorAnswer[(corPresent-1)] = "Black";
							  $( this ).dialog( "close" );
							  console.log(colorAnswer);
							},
						}
					}).prev(".ui-dialog-titlebar").css("background","#FFCC33");
				});
			});
