...
var showTreeStep = function(){
	...
	this.buildTree = function(){
		$(document).ready(function(){
			...
			$("#ShowAll").click(function(){
				if( nodeNoAVL >= 3 && nodeNoAVL <= 6 ){	
					$("#ShowAll").hide().delay(2800).show(0);
				}
				else if( nodeNoAVL <= 2 ){	
					$("#ShowAll").hide().delay(1300).show(0);
				}
				if( nodeNoAVL >= 7 && (document.getElementById("ShowAll").innerHTML == "Show AVL Tree") ){
					$("#ShowAll").remove();
					$("#getStep").remove();
				}
			});
		...
		//Tip of rotate tree.
			...
			this.rotateTree(checkIndex);
			document.getElementById("unbalance").innerHTML = "不平衡的情況為：none";
			document.getElementById("ShowAll").innerHTML = "Show AVL Tree";
		} 
		//
		////Press "Show All" button from AVL_Simulation(Insert).html.
		if( nodeNoAVL >= 7 && (document.getElementById("ShowAll").innerHTML == "Show AVL Tree") ){
			$(document).ready(function(){
				$(".Traversal").fadeIn();
				$("#ShowAll").hide();
	...
	this.changeBF = function(currentIndex){
		...
		console.log(checkBFArr);
				
		for( var j = 0; j <= checkBFArr.length; j++ ){
			checkIndex = checkBFArr[j];
			
			if( bfArray[(checkIndex-1)] == 2 ){              //L
				if( bfArray[((checkBFArr[j-1])-1)] == 1 ){     //LL
					document.getElementById("unbalance").innerHTML = "不平衡的情況為：LL";
				}
				else if( bfArray[((checkBFArr[j-1])-1)] == -1 ){     //LR
					document.getElementById("unbalance").innerHTML = "不平衡的情況為：LR";
				}
				document.getElementById("ShowAll").innerHTML = "Rotate Tree";
				isRotate = true;
				break;
			}
			else if( bfArray[(checkIndex-1)] == -2 ){       //R
				if( bfArray[((checkBFArr[j-1])-1)] == 1 ){     //RL
					document.getElementById("unbalance").innerHTML = "不平衡的情況為：RL";
				}
				else if( bfArray[((checkBFArr[j-1])-1)] == -1 ){      //RR
					document.getElementById("unbalance").innerHTML = "不平衡的情況為：RR";
				}
				document.getElementById("ShowAll").innerHTML = "Rotate Tree";
				isRotate = true;
				break;	
		...
		if( bfArray[(vIndex-1)] == 2 ){       //L
			...
			if( bfArray[(lIndex-1)] == 1 || bfArray[(lIndex-1)] == 2 ){ 
				...
				if( changeChildrenArr.length != 0 ){
					rotatePosition.rotateChildren(kindOfCase);
				}
			}
